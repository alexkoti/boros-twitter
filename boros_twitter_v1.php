...
class BorosTwitter {
		...
		switch( $this->config['type'] ){
			case 'search':
				...
				if( empty($this->config['search']) ){
					...
					$this->json_url = "http://search.twitter.com/search.json?q={$search_string}&rpp={$this->config['number']}&lang=pt";
			...
			case 'list':
				...
				if( $this->config['user'] == 'twitter' or empty($this->config['user']) ){
					...
					$this->json_url = "http://api.twitter.com/1/lists/statuses.json?owner_screen_name={$this->config['user']}&slug={$this->config['list']}&per_page={$this->config['number']}";
			...
			case 'user':
				if( $this->config['title'] == '' )
					$this->config['title'] = 'Twitter %USER%';
				$this->json_url = "http://api.twitter.com/1/statuses/user_timeline.json?screen_name={$this->config['user']}&include_rts={$this->config['include_rts']}&count={$this->config['number']}";
	 ...
	 */
	function get_tweets(){
		$json = wp_remote_get($this->json_url);
		
		// retornar em caso de erro de leitura(is_wp_error)
		if( is_wp_error( $json ) ){
			$errors = array();
			foreach( $json->errors as $error ){
				$errors[] = $error;
			}
			$this->errors['wp'] = $errors;
		...
		// separar os tweets da requisição
		$twitter_data = json_decode($json['body'], true);
		
		// caso de requisição htttp correta, porém com erro de resultado(ex 404)
		if( isset($twitter_data['error']) ){
			$this->errors['twitter_error'] = $twitter_data['error'];
		}
	...
	function process_search_tweets( $data ){
		foreach( $data['results'] as $tweet ){
			...
			$this->tweets[] = array(
				...
				'permalink' => 'http://twitter.com/#!/'. $tweet['from_user'] .'/status/'. $tweet['id_str'],
